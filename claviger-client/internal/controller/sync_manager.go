...
// Define an interface so we don't need to import vpn
type VPNController interface {
	HotSwapEndpoint(key, endpoint string) error
	GetState() string
}

// StartSyncManager launches the background synchronization engine.
// Notice we now pass the embedded *Engine !
func StartSyncManager(ctx context.Context, vault *config.ClientVault, vpn VPNController) {
	...
	go func() {
		for {
			...
			// if vpnEngine.GetState() != vpn.StateSecured {
			...
			if serverRev > currentRev {
				...
				// err = vpnEngine.HotSwapEndpoint(profile.ServerKey, state.Endpoint)
				// if err != nil {
