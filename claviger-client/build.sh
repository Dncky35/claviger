...
env -u CC CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -tags=headless -ldflags="-s -w" -o release/claviger-client-amd64 .
...
env -u CC CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -tags=headless -ldflags="-s -w" -o release/claviger-client-arm64 .
