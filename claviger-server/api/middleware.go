...
func HubAccessMiddleware(db *sql.DB, next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// ==========================================
		// BRANCH A: SERVER-TO-SERVER (Master Node)
		// ==========================================

		masterIP := storage.GetConfig(db, "master_vpn_ip")

		if masterIP != "" && clientIP == masterIP {
			expectedKey := storage.GetConfig(db, "node_secret")
			if expectedKey == "" {
				http.Error(w, "Node not paired. Remote access disabled.", http.StatusServiceUnavailable)
				return
			}
			...
			if authHeader != expectedHeader {
				log.Printf("🔒 Blocked: Master IP (%s) provided invalid Node Secret", clientIP)
				http.Error(w, "401 Unauthorized - Invalid Node Secret", http.StatusUnauthorized)
				return
			}

			// Passed Server-to-Server Auth! Let it through immediately.
			next.ServeHTTP(w, r)
			return
		}

		// ==========================================
		// BRANCH B: LOCALHOST FAILSAFE
		// ==========================================
		...
		// ==========================================
		// BRANCH C: HUMAN USER AUTH (Roles & MFA)
		// ==========================================
